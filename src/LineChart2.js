...
import { delay, filter, first, share, switchMap } from "rxjs/operators";
import "./App.css";
...
const source = fromEvent(el, "mousedown").pipe(
  switchMap(e => {
    ...
    // mouseup イベントの
    // どちらか早く発火した方を次へ流す
    return merge(of(e).pipe(delay(200)), fromEvent(el, "mouseup")).pipe(
      first()
    );
  }),
  share()
...
const click = source.pipe(filter(e => e.type === "mouseup")).subscribe(e => {
  alert("click！");
});

const drag = source.pipe(filter(e => e.type !== "mouseup")).subscribe(e => {
  alert("drag！");
});
    ...
    return (
      <div className="mod-graph" id="graph">
        <svg
          ...
          width="600"
          height="400"
          viewBox={`0 0 ${defaultSize.x} ${defaultSize.y}`}
        >
          <g
            className="graphBody"
            transform={`translate(${graphBody.left}, ${graphBody.top})`}
          >
            {/* x軸方向のメモリ */}
            <g className="ticksX">
              {xTicksPosList.map((pos, idx) => (
                <g
                  ...
                  key={pos}
                >
                  <line x1="0" y1="0" x2={graphBodyWidth} y2="0" />
                </g>
              ))}
            </g>
            ...
            <g className="ticksY">
              {yTicksPosList.map((pos, idx) => (
                <g
                  ...
                  key={pos}
                >
                  <line x1="0" y1="0" x2="0" y2={graphBodyHeight} />
                </g>
              ))}
            </g>
          ...
          >
            {xLabelPosList.map((pos, idx) => (
              <g
                className="tick"
                transform={`translate(${pos}, 0)`}
                key={dataset[idx].date}
              >
                <text x="5" y={10 + axis.strokeWidth} transform="rotate(45)">
                  {dataset[idx].date}
                </text>
              </g>
            ))}
          ...
          <g className="labelY" transform={`translate(0, ${graphBody.top})`}>
            {yLabelPosList.map((pos, idx) => (
              <g
                ...
                key={yLabelList[idx]}
              >
                <text x={graphBody.left - 5} y="0" textAnchor="end">
                  {yLabelList[idx]}
                </text>
              </g>
            ))}
